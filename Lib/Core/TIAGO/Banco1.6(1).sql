...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`Professor` (
  ...
  `senha` VARCHAR(70) CHARACTER SET 'latin1' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`Aluno` (
  ...
  `senha` VARCHAR(70) CHARACTER SET 'latin1' NOT NULL,
  `idCursoFK` INT(11) NOT NULL,
  `serie` VARCHAR(5) CHARACTER SET 'latin1' NOT NULL,
  PRIMARY KEY (`idAluno`),
...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`HorarioDisponivel` (
  `idHorarioDisponivel` INT(11) NOT NULL AUTO_INCREMENT,
  `dataHora` DATETIME NOT NULL,
  `idProfessorFK` INT(11) NULL DEFAULT NULL,
  PRIMARY KEY (`idHorarioDisponivel`),
  UNIQUE INDEX `idHorarioDisponivel_UNIQUE` (`idHorarioDisponivel` ASC),
  INDEX `FKProfessorAgendamento` (`idProfessorFK` ASC),
  CONSTRAINT `FKProfessor_Agendamento`
    FOREIGN KEY (`idProfessorFK`)
    REFERENCES `estartho_bdunp`.`Professor` (`idProfessor`))
...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`Usuario` (
  ...
  `idNiveisUsuarioFK` INT(11) NULL DEFAULT NULL,
  PRIMARY KEY (`idUsuario`),
  UNIQUE INDEX `idUsuario_UNIQUE` (`idUsuario` ASC),
  UNIQUE INDEX `matricula_UNIQUE` (`matricula` ASC),
  UNIQUE INDEX `login_UNIQUE` (`login` ASC),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC)
  CONSTRAINT `FKNiveisUsuario_Usuario`
    FOREIGN KEY (`idNiveisUsuarioFK`)
    REFERENCES `estartho_bdunp`.`NiveisUsuario` (`idNiveisUsuario`)
    ON UPDATE CASCADE,)
ENGINE = InnoDB
AUTO_INCREMENT = 1
DEFAULT CHARACTER SET = latin1
...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`Defesa` (
  ...
  `idProfessorFK1` INT(11) NULL DEFAULT NULL,
  `idProfessorFK2` INT(11) NULL DEFAULT NULL 
  ...
  CONSTRAINT `FKTCC_Defesa`
    ...
    REFERENCES `estartho_bdunp`.`TCC` (`idTCC`)
    ON UPDATE CASCADE)
  CONSTRAINT `FKProfessor1_Defesa`
    FOREIGN KEY (`idProfessorFK1`)
    REFERENCES `estartho_bdunp`.`Professor` (`idProfessor`)
  CONSTRAINT `FKProfessor2_Defesa`
    FOREIGN KEY (`idProfessorFK2`)
    REFERENCES `estartho_bdunp`.`Professor` (`idProfessor`)  
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1
COLLATE = latin1_general_ci;
...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`NiveisUsuario` (
  `idNiveisUsuario` INT(11) NOT NULL AUTO_INCREMENT,
  `nivel` VARCHAR(15) CHARACTER SET 'latin1' NOT NULL,
  PRIMARY KEY (`idNiveisUsuario`),
  UNIQUE INDEX `idNiveisUsuario_UNIQUE` (`idNiveisUsuario` ASC),
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1
COLLATE = latin1_general_ci;

CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`ProfessorExterno` (
  `idProfessorExterno` INT(11) NOT NULL AUTO_INCREMENT,
  `nome` VARCHAR(100) CHARACTER SET 'latin1' NOT NULL,
  `idDefesaFK` INT (11) NULL DEFAULT NULL
  PRIMARY KEY (`idProfessorExterno`),
  UNIQUE INDEX `idProfessorExterno_UNIQUE` (`idProfessorExterno` ASC),
  CONSTRAINT `FKDefesa_ProfessorExterno`
    FOREIGN KEY (`idDefesaFK`)
    REFERENCES `estartho_bdunp`.`Defesa` (`idDefesa`)
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 1
...
CREATE TABLE IF NOT EXISTS `estartho_bdunp`.`TCCProfessorExterno` (
  `idProfessorExternoFK` INT(11) NOT NULL,
  `idTCCFK` INT(11) NOT NULL,
  INDEX `FKProfessorExternoTCCProfessorExterno` (`idProfessorFK` ASC),
  INDEX `FKTCCComponente` (`idTCCFK` ASC),
  UNIQUE INDEX `idTCCFK_UNIQUE` (`idTCCFK` ASC),
  CONSTRAINT `FKProfessorExterno_TCCProfessorExterno`
    FOREIGN KEY (`idProfessorExternoFK`)
    REFERENCES `estartho_bdunp`.`ProfessorExterno` (`idProfessorExterno`),
  CONSTRAINT `FKTCC_TCCProfessorExterno`
    FOREIGN KEY (`idTCCFK`)
    REFERENCES `estartho_bdunp`.`TCC` (`idTCC`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_alterarAluno`(idAluno integer(11) ,nome varchar(100), matricula char(9), email varchar(50), 
  login varchar(50), senha varchar(70), idCursoFK integer(11), serie varchar(5))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_alterarDefesa`(idDefesa integer(11), DataDef DATETIME, idTCCFK integer(11), idReservaFK integer(11), idProfessorFK1 integer, idProfessorFK2, integer)
BEGIN 
...
`idProfessorFK1` = idProfessorFK1,
`idProfessorFK2` = idProfessorFK2
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_alterarProfessor`(idProfessor integer(11), nome varchar(100), matricula integer(6), email varchar(50), login varchar(50), senha varchar(70))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_alterarSala`(idSala integer(11), setor varchar(10), nome varchar (20), idUnidadeFK integer(11))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_alterarTCC`(idTCC integer(11), titulo varchar(100), resumo varchar(250), statusTCC tinyint(1), objetivo varchar(100), 
justificativa varchar(100), TCCTipo tinyint(1), idLinhaPesquisaFK integer(11), idProfessorFK INTEGER (11), idAlunoFK1 INTEGER (11), idAlunoFK2 INTEGER (11))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_inserirAluno`(nome varchar(100), matricula char(9), email varchar(50), login varchar(50), senha varchar(70), 
  idCursoFK integer(11), serie varchar(5))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_inserirDefesa`(DataDef DATETIME, idTCCFK integer(11), idReservaFK integer(11), idProfessorFK1 integer(11), 
idProfessorFK2 integer(11))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_inserirProfessor`(nome varchar(100), matricula char(6), email varchar(50), login varchar(50), senha varchar(70))
BEGIN 
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_inserirProfessorExterno`(nome varchar(100))
BEGIN 
 ...
 START TRANSACTION;
  ...
  INSERT INTO `estartho_bdunp`.`ProfessorExterno`
(`nome`
)
VALUES
(nome);
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_inserirSala`(setor varchar(10), nome varchar (20), idUnidadeFK integer)
BEGIN 
...
BEGIN
SELECT C.`idCurso` AS ID, C.`nomeCurso` AS `Curso` FROM `estartho_bdunp`.`Curso` AS C;
END$$
...
BEGIN
SELECT U.`nome` AS `Unidade`, S.`setor` AS Setor, S.`nome` AS Sala, S.`idSala` AS ID FROM `estartho_bdunp`.`Sala` AS S
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_buscarProfessor`(nome VARCHAR(100))
BEGIN
SELECT P.`idProfessor` AS `ID`, P.`nome` AS `Professor`, P.`matricula`, P.`email`, P.`login`, P.`senha`, T.`titulo` AS `TCC`, LP.`nome` AS `Linha de Pesquisa` FROM `estartho_bdunp`.`Professor` AS P 
INNER JOIN `estartho_bdunp`.`Componentes` AS C ON P.`idProfessor` = C.`idProfessorFK`
INNER JOIN `estartho_bdunp`.`TCC` AS T ON T.`idTCC` = C.`idTCCFK`
INNER JOIN `estartho_bdunp`.`LinhaPesquisa` AS LP ON LP.`idLinhaPesquisa` = T.`idLinhaPesquisaFK`
where `estartho_bdunp`.`Professor`.`nome` = nome
ORDER BY T.`titulo` ASC;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_buscarAluno`(nome VARCHAR(100))
BEGIN
SELECT A.`idAluno` AS `ID`, A.`nome` AS `Professor`, A.`matricula`, A.`email`, A.`login`, A.`senha`, C.`nome` AS `Curso`, P.`nome` AS `Professor`, T.`titulo` AS `TCC`, LP.`nome` AS `Linha de Pesquisa` FROM `estartho_bdunp`.`Aluno` AS A 
INNER JOIN `estartho_bdunp`.`Curso` AS C ON C.`idCurso` = A.`idCursoFK`
INNER JOIN `estartho_bdunp`.`Componentes` AS Co ON Co.`idAlunoFK` = A.`idAluno` 
INNER JOIN `estartho_bdunp`.`TCC` AS T ON T.`idTCC` = Co.`idTCCFK`
INNER JOIN `estartho_bdunp`.`Professor` AS P ON P.`idProfessor` = Co.`idProfessorFK`
INNER JOIN `estartho_bdunp`.`LinhaPesquisa` AS LP ON LP.`idLinhaPesquisa` = T.`idLinhaPesquisaFK`
where `estartho_bdunp`.`Aluno`.`nome` = nome;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_buscarTCC`(titulo varchar(100))
BEGIN
SELECT T.`idTCC` AS `ID`, T.`titulo` AS `Titulo do TCC`, T.`resumo`  AS `Resumo`, T.`statusTCC` AS `Status do TCC`, T.`objetivo` AS `Objetivo`, T.`justificativa` AS `Justificativa`, T.`TCCTipo`  AS `Tipo de TCC`, LP.`nome` AS `LinhaPesquisa`, P.`nome` AS `Professor`, A.`nome` AS `Aluno1`, A1.`nome` AS `Aluno2` FROM `estartho_bdunp`.`TCC` AS T 
INNER JOIN `estartho_bdunp`.`LinhaPesquisa` AS LP ON LP.`idLinhaPesquisa`= T.`idLinhaPesquisaFK` 
INNER JOIN `estartho_bdunp`.`Componentes` AS C ON T.`idTCC` = C.`idTCCFK` 
INNER JOIN `estartho_bdunp`.`Professor` AS P ON P.`idProfessor` = C.`idProfessorFK` 
INNER JOIN `estartho_bdunp`.`Aluno` AS A ON  A.`idAluno` = C.`idAlunoFK1`
INNER JOIN `estartho_bdunp`.`Aluno` AS A1 ON  A1.`idAluno` = C.`idAlunoFK2`
WHERE `estartho_bdunp`.`TCC`.`titulo` = titulo;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_buscarTCC_LPesquisa`(nome varchar(250))
BEGIN
SELECT LP.`nome` AS `LinhaPesquisa`, T.`titulo` AS `Titulo do TCC`, T.`idTCC` AS `ID`, T.`resumo`  AS `Resumo`, T.`statusTCC` AS `Status do TCC`, T.`objetivo` AS `Objetivo`, T.`justificativa` AS `Justificativa`, T.`TCCTipo`  AS `Tipo de TCC`, P.`nome` AS `Professor`, A.`nome` AS `Aluno1`, A1.`nome` AS `Aluno2` FROM `estartho_bdunp`.`LinhaPesquisa` AS LP 
INNER JOIN `estartho_bdunp`.`TCC` AS T ON T.`idLinhaPesquisaFK` = LP.`idLinhaPesquisa` 
INNER JOIN `estartho_bdunp`.`Componentes` AS C ON T.`idTCC` = C.`idTCCFK` 
INNER JOIN `estartho_bdunp`.`Professor` AS P ON P.`idProfessor` = C.`idProfessorFK` 
INNER JOIN `estartho_bdunp`.`Aluno` AS A ON  A.`idAluno` = C.`idAlunoFK1`
INNER JOIN `estartho_bdunp`.`Aluno` AS A1 ON  A1.`idAluno` = C.`idAlunoFK2`
WHERE `estartho_bdunp`.`LinhaPesquisa`.`nome` = nome
ORDER BY T.`titulo` ASC;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_buscarTCC_data`(titulo varchar(100))
BEGIN
SELECT D.`data` AS `Data da Defesa` ,T.`idTCC` AS `ID`, T.`titulo` AS `Titulo do TCC`, T.`resumo`  AS `Resumo`, T.`statusTCC` AS `Status do TCC`, T.`objetivo` AS `Objetivo`, T.`justificativa` AS `Justificativa`, T.`TCCTipo`  AS `Tipo de TCC`, LP.`nome` AS `LinhaPesquisa`, P.`nome` AS `Professor`, A.`nome` AS `Aluno1`, A1.`nome` AS `Aluno2`FROM `estartho_bdunp`.`TCC` AS T 
INNER JOIN `estartho_bdunp`.`LinhaPesquisa` AS LP ON LP.`idLinhaPesquisa`= T.`idLinhaPesquisaFK` 
INNER JOIN `estartho_bdunp`.`Componentes` AS C ON T.`idTCC` = C.`idTCCFK` 
INNER JOIN `estartho_bdunp`.`Professor` AS P ON P.`idProfessor` = C.`idProfessorFK` 
INNER JOIN `estartho_bdunp`.`Aluno` AS A ON  A.`idAluno` = C.`idAlunoFK1`
INNER JOIN `estartho_bdunp`.`Aluno` AS A1 ON  A1.`idAluno` = C.`idAlunoFK2`
INNER JOIN `estartho_bdunp`.`Defesa` AS D ON D.`idTCCFK` = T.`idTCC`
WHERE `estartho_bdunp`.`TCC`.`titulo` = titulo
ORDER BY D.`data` ASC;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_listarAgendamento_data`(dataHora datetime)
BEGIN
SELECT Ag.`dataHora` AS `Dia e Hora`, Ag.`motivo` AS `Motivo do Agendamento`, Ag.`idAgendamento` AS `ID`, P.`nome` AS `Professor`, A.`nome` AS `Aluno` FROM `estartho_bdunp`.`Agendamento` AS Ag
INNER JOIN `estartho_bdunp`.`Professor` AS P ON P.`idProfessor` = Ag.`idProfessorFK`
INNER JOIN `estartho_bdunp`.`Aluno` AS A ON A.`idAluno` = Ag.`idAlunoFK`
WHERE `estartho_bdunp`.`Agendamento`.`dataHora` = dataHora
ORDER BY dataHora DESC;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_listarAgendamento_Prof`(nome varchar(100))
BEGIN
SELECT P.`nome` AS `Professor`, Ag.`dataHora` AS `Dia e Hora`, Ag.`motivo` AS `Motivo do Agendamento`, Ag.`idAgendamento` AS `ID`, A.`nome` AS `Aluno`  FROM `estartho_bdunp`.`Professor` AS P
INNER JOIN `estartho_bdunp`.`Agendamento` AS Ag ON Ag.`idProfessorFk` = P.`idProfessor`
INNER JOIN `estartho_bdunp`.`Aluno` AS A ON A.`idAluno` = Ag.`idAlunoFK`
WHERE `estartho_bdunp`.`Professor`.`nome` = nome
ORDER BY dataHora DESC;
END$$
...
DELIMITER $$
USE `estartho_bdunp`$$
CREATE PROCEDURE `sp_listarAgendamento_Aluno`(nome varchar(100))
BEGIN
SELECT A.`nome` AS `Aluno`, Ag.`dataHora` AS `Dia e Hora`, Ag.`motivo` AS `Motivo do Agendamento`, Ag.`idAgendamento` AS `ID`, P.`nome` AS `Professor` FROM `estartho_bdunp`.`Aluno` AS A
INNER JOIN `estartho_bdunp`.`Agendamento` AS Ag ON Ag.`idAlunoFk` = A.`idAluno`
INNER JOIN `estartho_bdunp`.`Professor` AS P ON P.`idProfessor` = Ag.`idProfessorFK`
WHERE `estartho_bdunp`.`Aluno`.`nome` = nome
ORDER BY dataHora DESC;
END$$
