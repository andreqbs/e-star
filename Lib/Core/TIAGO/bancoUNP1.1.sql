...
BEGIN
SELECT * FROM`estartho_bdunp`.`LinhaPesquisa` ORDER BY nome ASC;
END$$
...
BEGIN
SELECT * FROM `estartho_bdunp`.`TCC` ORDER BY  titulo ASC;
END$$
...
BEGIN
SELECT `link`,DISTINCT (`idTCCFK`) ,`versao` FROM `estartho_bdunp`.`Arquivo` ORDER BY DESC;
END$$
...
BEGIN
SELECT * FROM `estartho_bdunp`.`Atividade` ORDER BY MES DESC;
END$$
...
BEGIN
SELECT * FROM `estartho_bdunp`.`Sala` AS S INNER JOIN `estartho_bdunp`.`Unidade` AS U ON S.`idUnidadeFK` = U.`idUnidade` ORDER BY U.`nome` ASC, S.`nome` ASC;
END$$
...
BEGIN
SELECT * FROM `estartho_bdunp`.`Reserva` ORDER BY dataHora DESC;
END$$
...
BEGIN
SELECT * FROM `estartho_bdunp`.`Erros` ORDER BY nomeErro ASC;
END$$
...
R.`TipoRelatorio`, R.`apresentacaoComentario`, R.`fundamentacaoComentario`, R.`desenvolvimentoComentario`, R.`resultadosComentario`
 FROM `estartho_bdunp`.`Relatorio` AS R INNER JOIN `estartho_bdunp`.`TCC` AS T ON R.`idTCCFK` = T.`idTCC` ORDER BY R.`data` ASC, T.`titulo` ASC;
END$$
...
BEGIN
SELECT * FROM `estartho_bdunp`.`AlunoDefesa`order by idAluno ASC;
END$$
...
AD.`comunicacaoComentario`, AD.`clarezaComentario`, AD.`dominioComentario`, AD.`idDefesa`
 FROM `estartho_bdunp`.`AlunoDefesa` AS AD INNER JOIN `estartho_bdunp`.`Aluno` AS A ON AD.`idAluno`= A.`idAluno` order by A.`nome` ASC;
END$$
