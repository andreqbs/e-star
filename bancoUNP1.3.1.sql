...
CREATE INDEX `FKCursoLinhaPesquisa` ON `e-star`.`LinhaPesquisa` (`idCursoFK` ASC);
...
CREATE INDEX `FKLinha_Pesquisa_TCC` ON `e-star`.`TCC` (`idLinhaPesquisaFK` ASC);
...
CREATE INDEX `FKTCCArquivo` ON `e-star`.`Arquivo` (`idTCCFK` ASC);
...
CREATE INDEX `FKTCCAtividade` ON `e-star`.`Atividade` (`idTCCFK` ASC);
...
CREATE INDEX `FKProfessorComponente` ON `e-star`.`Componentes` (`idProfessorFK` ASC);

CREATE INDEX `FKAlunoComponente` ON `e-star`.`Componentes` (`idAlunoFK1` ASC);

CREATE INDEX `FKAlunoComponente_2` ON `e-star`.`Componentes` (`idAlunoFK2` ASC);

CREATE INDEX `FKTCCComponente` ON `e-star`.`Componentes` (`idTCCFK` ASC);
...
CREATE INDEX `FKProfessorReserva` ON `e-star`.`Reserva` (`idProfessorFK` ASC);

CREATE INDEX `FKSalaAulaReserva` ON `e-star`.`Reserva` (`idSalaFK` ASC);
...
CREATE INDEX `FKTCCDefesa` ON `e-star`.`Defesa` (`idTCCFK` ASC);

CREATE INDEX `FKReservaDefesa` ON `e-star`.`Defesa` (`idReservaFK` ASC);
...
CREATE INDEX `FKTCCErrosTCC` ON `e-star`.`ErrosTCC` (`idTCCFK` ASC);

CREATE INDEX `FKErrosErrosTCC` ON `e-star`.`ErrosTCC` (`idErrosFK` ASC);
...
CREATE INDEX `FKProfessorEvento` ON `e-star`.`Evento` (`idProfessorFK` ASC);

CREATE INDEX `FKCursoEvento` ON `e-star`.`Evento` (`idCursoFK` ASC);
...
CREATE INDEX `FKTCCRelatorio` ON `e-star`.`Relatorio` (`idTCCFK` ASC);
...
CREATE INDEX `fk_AlunoDefesaAluno1_idx` ON `e-star`.`AlunoDefesa` (`idAluno` ASC);

CREATE INDEX `fk_AlunoDefesa_Defesa1idx` ON `e-star`.`AlunoDefesa` (`idDefesa` ASC);
...
=======
  UPDATE `e-star`.`Professor`
    SET 
>>>>>>> 3df1b8743390d3375e919f527d8a3a5863e27a15
`nome`= nome,
`matricula`= matricula,
`email`= email,
`login`=login,
...
-- ------------------------------------------------------
DROP procedure IF EXISTS `e-star`.`sp_listarRelatorio`;
DELIMITER $$
